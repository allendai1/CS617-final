...
let dropdown = d3.select(".selection").on("change", (e) => {
  ...
  var d = d3.select(`path[county=${currentCounty}]`);
  const [[x0, y0], [x1, y1]] = path.bounds(d.data()[0]);
...
var sp2 = d3
	...
	.append("g")
	.attr("transform", "translate(" + margin.left + "," + margin.top + ")");
var xAxisMMYYYY = d3.scaleLinear().domain([2016, 2024]).range([0, w]);
var yAxisMMYYYY = d3.scaleLinear().domain([0, 3000]).range([h, 0]);

sp2
	.append("g")
	.attr("transform", "translate(0," + h + ")")
	.call(d3.axisBottom(xAxisMMYYYY));
sp2.append("g").call(d3.axisLeft(yAxisMMYYYY));

// arr of objects where each object is {listings: 1234, price : 1234}
data.then((x) => {
	let county_price_per_sqft = {};
	x.forEach((e) => {
		if (e.county_name in county_price_per_sqft) {
			county_price_per_sqft[e.county_name].push([
				e.month_date_yyyymm.substring(0, 4),
				e.median_listing_price_per_square_foot,
			]);
		} else {
			county_price_per_sqft[e.county_name] = [
				[
					e.month_date_yyyymm.substring(0, 4),
					e.median_listing_price_per_square_foot,
				],
	...
	// console.log(county_price_per_sqft);
	let newObj = {};
	// {"hampshire" : {"2024": 42224, "2023" : 53353}}
	for (const county of Object.keys(county_price_per_sqft)) {
		// Initialize an empty object to store prices by year for the current county
		newObj[county] = {};

		// Loop through each pair (year, price) in the current county
		for (const pair of county_price_per_sqft[county]) {
			const yyyy = pair[0];
			const price = parseInt(pair[1]);

			// Check if the year key exists for the current county
			if (!newObj[county][yyyy]) {
				// If it doesn't exist, initialize an array to store prices for that year
				newObj[county][yyyy] = [];
			}

			// Push the price into the array for the current year
			newObj[county][yyyy].push(price);
		}
	}
	// console.log(newObj);
